...
# Revised : Kimman
# The program is revised so that the program can run on machines with limited resources
#
# run this program as a batch
# run.sh
# python music_rbm.py new
# python music_rbm.py continue
# python music_rbm.py continue
# python music_rbm.py continue
# python music_rbm.py generate 
...
sys.path.append('/notebooks/torch/')


from lib_midi.utils import midiread, midiwrite
import theano
...
def shared_normal(num_rows, num_cols, scale=1):
    ...
    return theano.shared(numpy.random.normal(
        scale=scale, size=(num_rows, num_cols)).astype(theano.config.floatX) ,borrow=True)


def shared_zeros(*shape):
    '''Initialize a vector shared variable with zero elements.'''
    return theano.shared(numpy.zeros(shape, dtype=theano.config.floatX) ,borrow=True)
...
class RnnRbm:
    ...
    def train(self, files, batch_size=100, num_epochs=200):
        ...
        if self.lastcost != None:
            print("last cost:")
            print(numpy.mean(self.lastcost ))    
            ...
            for epoch in range(num_epochs):
                ...
                for s, sequence in enumerate(dataset): 
                    ...
                    for i in range(0, len(sequence), batch_size):
                        tmp=sequence[i:i + batch_size]
                        lentmp=len(tmp)
                        cost = self.train_function(tmp)
                        costs.append(cost)
                        
                f = open('RnnRbmObj.save', 'wb')
...
def test_rnnrbm(batch_size=100, num_epochs=200, loadFile=None, continueToTrain=True):   
    if loadFile!=None:
        print("loading obj from "+loadFile); 
        f = open(loadFile, 'rb')
    ...
    re = "/notebooks/torch/biaxial-rnn-music-composition-master/music/*.mid"
    if continueToTrain==True or loadFile==None:
...
if __name__ == '__main__':
    ...
    loadFile='RnnRbmObj.save'
    continueToTrain=True
    
    if modeV=="new":
        loadFile=None 
    if modeV=="generate":
        continueToTrain=False
    
    print(modeV)
    
    model = test_rnnrbm(batch_size=50, num_epochs=200, loadFile=loadFile, continueToTrain=continueToTrain)
    print("start doing model.generate('sample1.mid')")
    model.generate('sample1.mid')
    print("start doing model.generate('sample2.mid')")
